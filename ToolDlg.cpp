...
BEGIN_MESSAGE_MAP(CToolDlg, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_CBO_SERIAL_PORT, &CToolDlg::OnCbnSelchangeCboSerialPort)
	...
	if (!m_Light)
	{
		...
		if ((m_RxData[m_iRxLen - 2] == '\r' && m_RxData[m_iRxLen - 1] == '\n'))
		{
			...
			if (msgTemp == msgNotice.Mid(0, 10))
			{
				...
				str2hex(m_HexData, hexData);		//维护时str2hex可能需要更改参数
				...
				*/
				...
				//m_CtrRichEdit.LineScroll(nline - 1);				
			}
...
void CToolDlg::str2hex(char* str, char* hex)	//数据长度250 这个函数可能需要更改长度
{
...
void CToolDlg::OnBnClickedDefault()
{
	if (m_Connected)
	{
		...
		defaultName.Delete(0, 4);								//找到按钮的数字
		CString	idfName = defaultName.Left(1);
		...
		CString idfName2;
		idfName2.Format("%d", i);
		SetDlgItemText(IDC_BUTTON1, _T("设置") + _T(idfName2) + _T("通道默认"));
...
void CToolDlg::OnBnClickedCheck1()
{
	...
	{
		SetDlgItemText(IDC_BUTTON1 , _T("设置0通道默认"));
		m_Check2 = FALSE;
...
void CToolDlg::OnBnClickedCheck2()
{
	...
	{
		SetDlgItemText(IDC_BUTTON1 , _T("设置1通道默认"));
		m_Check1 = FALSE;
		m_Check3 = FALSE;
		m_Check4 = FALSE;
		m_Check5 = FALSE;
...
void CToolDlg::OnBnClickedCheck3()
{
	...
	{
		SetDlgItemText(IDC_BUTTON1 , _T("设置2通道默认"));
		m_Check1 = FALSE;
		m_Check2 = FALSE;
		m_Check4 = FALSE;
		m_Check5 = FALSE;
...
void CToolDlg::OnBnClickedCheck4()
{
	...
	{
		SetDlgItemText(IDC_BUTTON1 , _T("设置3通道默认"));
		m_Check1 = FALSE;
		m_Check2 = FALSE;
		m_Check3 = FALSE;
		m_Check5 = FALSE;
...
void CToolDlg::OnBnClickedCheck5()
{
	...
	{
		SetDlgItemText(IDC_BUTTON1 , _T("设置4通道默认"));
		m_Check1 = FALSE;
		m_Check2 = FALSE;
		m_Check3 = FALSE;
		m_Check4 = FALSE;
...
void CToolDlg::OnBnClickedCheck6()
{
	...
	{
		SetDlgItemText(IDC_BUTTON1 , _T("设置5通道默认"));
		m_Check1 = FALSE;
		m_Check2 = FALSE;
		m_Check3 = FALSE;
		m_Check4 = FALSE;
...
void CToolDlg::OnBnClickedCheck7()
{
	...
	{
		SetDlgItemText(IDC_BUTTON1 , _T("设置6通道默认"));
		m_Check1 = FALSE;
		m_Check2 = FALSE;
		m_Check3 = FALSE;
		m_Check4 = FALSE;
...
void CToolDlg::OnCbnSelchangeCboSerialPort()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData(TRUE);
	for(int i = 1; i < 200 ; ++i )
